test_that("2D-time interpolation of small data set", {
  ...
  expect_no_error(output_v <- interpolate_sparse(input_xytv = input_xytv, 
                                 output_xy  = output_xy, 
                                 output_t   = output_t  ))
})
...
test_that("2D interpolation of weather data - inverse distance", {
  ...
  expect_no_error(output_wind <- interpolate_sparse(input_xytv  = Input, 
                                  output_xy   = output_xy, 
                                  barycentric = FALSE,
                                  output_t    = output_t))
