...
read_KNMI <- function(file, dir = "",    # file and directory
                      attr = NULL, ...){ # attributes to add
  KNMI <- NULL
  for (fi in file){
    fn <- paste(dir, fi, sep="/")
  ...
  vars <- data.frame(
    variable = c("windspeed", "winddirection",
 ...
 description = c("Mean wind speed for the hour preceding the observation time stamp",
       ...
       unit = c("m/s", "dg", "dgC", "dgC", "J/m2/s", 
                "mbar (hPa)", "octants", "-", "dgC"),
   original = c("FH", "DD", "T", "TD", "Q", "P", "N", "U", "TZ"))
  
  stats <- merge(KNMIstations, 
                 list(station = unique(KNMI$station)))
...
read_bathymetry <- function(
        file, dir = "", 
        attr = NULL, 
        lonlim = NULL, latlim = NULL, # range in longitude, latitu e 
        levels = c( -5, 0, 5, 10, 20), 
        by = 1, ...){
  ...
  vars <- list(variable = c("longitude", "latitude", "depth"),
               unit     = c("dgE", "dgN", "m"),
               note = c("EPSG:4326", 
"original data: depth below the lowest astronomical tide, estimated as - elevation"))
