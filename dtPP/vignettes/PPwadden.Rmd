...
time <- seq(from = as.POSIXct("2021-01-05 01:00:00"), 
            to   = as.POSIXct("2021-03-05 01:00:00"), 
...
st235   <- subset(Weather2021, 
                  subset = 
                    station == 235                                &
                    datetime >= as.POSIXct("2021-01-04 23:59:59") &
                    datetime <= as.POSIXct("2021-03-05 01:00:10"))
...
ChlLong <- with(Chlor2021, 
                expand.grid(nr        = 1:nrow(Chlor2021$datetime), 
                            longitude = longitude, 
                            latitude  = latitude))

ChlLong$date.from   <- Chlor2021$datetime[ChlLong$nr, 1]
ChlLong$date.to     <- Chlor2021$datetime[ChlLong$nr, 2]
ChlLong$chlorophyll <- as.vector(Chlor2021$chlorophyll)
...
```{r}  
Ht.data  <- WadHeightHR[ , c("datetime", "DENHDR")]
Ht.data  <- subset(Ht.data, 
                  subset =                                         
                    datetime >= as.POSIXct("2021-01-04 23:59:59")  &
                    datetime <= as.POSIXct("2021-04-05 01:00:10"))

Tt.data  <- WadTempHR[ , c("datetime", "DENHDVSGR")]
...
```{r}
Chl_Stats <- unique(ChlLong[,c("longitude", "latitude")])
...
PPall <- cbind(PPall, PP$ts$ps)
}
colnames(PPall) <- paste(format(Chl_Stats$longitude, digits=3), "E_", 
...
matplot(PPall$datetime, PPall[,-1], type="l", 
        lty=1, ylab="mgC/m2/h", las=1, log="y",
        main="PP wadden")
```
